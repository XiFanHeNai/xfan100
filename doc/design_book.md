...
- PC地址：在上例中是*0x80000266*,该地址表明当前指令存储的位置。这个位置并不是在存储器中的地址，而是在总线系统中的地址。一般来讲，存储器在总线中有一个基地址，这个基地址是由编译过程中的链接文件指定的。然后编译器会计算出某些代码块的相对地址，这两个地址的和，才是最终该指令在总线系统中的地址。
  - 在当前的设计中，由于我们仅仅关注某一条指令在流水线中的具体执行过程，因此并不关心其PC地址的值。故在具体的仿真波形中，该信号是一个未知信号。
...
为保证流水线的执行速率，一般来讲指令都是存储在速度最快的存储器，例如RAM中的。取指模块的作用就是按需将这些指令取出来交给执行单元执行。此处，由于我们仅关注单个指令的执行过程，并不关心取指过程中的诸如分支预测，PC计算等操作，只是模拟指令从取指模块出去之后的一系列执行过程。因此目前取指模块基本上是一个空模块。详情请参见[xf100_ifu](xf100/rtl/core/ifu/xf100_ifu.v)。

```verilog
module xf100_ifu 
(
    output [`XF100_INSTR_SIZE-1:0] ifu_o_instr, //取到的指令码
    output [`XF100_PC_SIZE-1:0]    ifu_o_pc,    //指令对应的PC值
...
取指模块的输出波形如下图所示，由于此处我们并不关心PC的值，因此PC的输出信号为高阻态。

![simple ifu output wave -191118](doc_pic/simple ifu output wave -191118.jpg)
...
如果当前指令是一个未定义的非法指令，意味着需要产生异常，调用异常处理程序。如果当前指令码需要源操作数和目的操作数，则需要从寄存器文件(regfile)模块中读出相应的源操作数，并给出是否需要写回目的操作数的标志。其接口如下，具体代码请参考[xf100_exu_decode](xf100/rtl/core/exu/xf100_exu_decode.v)。
...
译码模块的接口波形如下所示。从图中可以看出，对于输入的指令码*0x00208f33*，该译码模块能正确识别出它是一个算术操作类型的指令，其具体指令操作包含在*dec_o_alu_info*信号组中。此外，该指令需要读取源操作数1和2寄存器，寄存器编号分别为*0x1*和*0x2*并且需要写回到目的寄存器，编号为*0x1e*。

![simple decode interface wave-191118](doc_pic/simple decode interface wave-191118.jpg)

### 预备操作数

操作数是存储在寄存器文件里的，由32个寄存器（目前设计只支持32bit指令集，因此其寄存器数目固定为32）组成。因此写寄存器在下一拍生效，读寄存器采用max设计，在当拍生效。此外，根据当前设计需求，我们仅仅实现一条简单的加法指令，因此只需要实现一个写回端口。但是由于其有两个源操作数，需要同时读两个不同的寄存器，因此需要两个读端口。其接口如下，具体代码请参考[xf100_exu_regfile](xf100/rtl/core/exu/xf100_exu_regfile.v)。
...
寄存器读取在本样例中暂时没有用到读操作，因为其数据是直接指定的。所以此处不展示其读接口的波形。但是该本例中，加法指令需要写回到目的寄存器中，因此需要注意其写端口的波形，如下图所示。图中可以看出，寄存器写使能信号在当前加法指令执行时期是有效的，表明该指令确实需要写回。并且写回寄存器的编号是*0x1e*,这也与*xf100_exu_decode*模块的输出信号想符合。最后，需要写回的结果数据是*0x300*,也符合我们的预期。***（我们预设源操作数1和2分别为0x100和0x200）***。

![simple regfile write port wave-191118](doc_pic/simple regfile write port wave-191118.jpg)

### 执行命令

执行模块负责具体指令的执行操作，比如在本例中，加法的执行部分就在此模块中。当所有的数据准备好时，就可以执行指令操作了。执行模块的接口如下，具体代码请参考[xf100_exu_alul](xf100/rtl/core/exu/xf100_exu_alu.v)。事实上，从上图中可以看出，加法计算确实被执行，其执行后的结果也是正确的。
...
写回是一条指令最后的一个步骤，当指令执行完毕，如果需要将结果写回，就会使能写回控制信号，将结果写回到寄存器文件中供下一条指令使用。此外，写回控制还涉及到仲裁的问题，不过在当前设计中，由于我们只实现了一条指令，因此不存在仲裁问题。在本例中，写回模块的输入与输出，是直接穿透的。写回模块的接口定义如下，具体代码参见[xf100_exu_wbck](xf100/rtl/core/exu/xf100_exu_wbck.v)。
...
![simple alu in exu  waveform-191118](doc_pic/simple alu in exu  waveform-191118.jpg)

从上图中可以看出，加法指令被成功执行，并被写回到了相应的寄存器中。以上就是一条加法指令在CPU中执行的过程。
